...
data = json.load(open("Final_Ticket_messages_LeftJOin.json"))
...
totalunique_ticket_ids_list = list(unique_ticket_ids)

totalunique_ticket_ids_list = sorted(totalunique_ticket_ids_list, key=lambda x: random.random())
print(totalunique_ticket_ids_list)
print("Total No of Chats : ",len(totalunique_ticket_ids_list))
print("Dividing chats in to 12 parts")
unique_ticket_ids_list_part = len(totalunique_ticket_ids_list)//12
for i in range(12):
    print("*"*40,i,"*"*40)
    unique_ticket_ids_list = totalunique_ticket_ids_list[i*unique_ticket_ids_list_part:(i+1)*unique_ticket_ids_list_part]
    print(f"Total {i} file length is {len(unique_ticket_ids_list)}")
    ...
    with open(f'training_data_{len(training_data)}_{i}.jsonl', 'w') as jsonl_file:
    ...
    with open(f'validation_data_{len(validation_data)}_{i}.jsonl', 'w') as jsonl_file:
        for entry in validation_data:
            jsonl_file.write(json.dumps(entry) + '\n')
    end_time  = time.time()
    time1 = end_time - start_time
    #time in minutes
    print("Time Taken: ", time1/60)
